...
function Badge({ n, active }: { n?: number; active: boolean }) {
  if (!n) return null;
  return <span className={cn("rounded-full px-2 py-0.5 text-[11px] font-bold tabular-nums", active ? "bg-white/20 text-white" : "bg-aqua-500 text-white")}>{n > 99 ? "99+" : n}</span>;
}
          ...
          <li key={item.href}>
            <Link
              ...
              className={cn(
                ...
                active ? "bg-aqua-500 text-white shadow-soft" : "text-navy-200 hover:bg-white/10 hover:text-white active:bg-white/15",
              )}
            >
            ...
            return (
              <li key={item.href}>
                <Link
                  ...
                  className={cn("relative flex min-h-14 flex-col items-center justify-center gap-1 text-[10px] font-semibold transition", active ? "text-aqua-700" : "text-navy-500 active:text-navy-900")}
                >
                  <span className={cn("flex h-7 w-11 items-center justify-center rounded-full transition", active && "bg-aqua-100")}>
                    <PendingIcon icon={item.icon} className="size-[18px]" />
                  </span>
                  {item.short ?? item.label}
                  {n ? <span className="absolute right-[calc(50%-1.6rem)] top-1.5 min-w-4 rounded-full bg-aqua-500 px-1 text-center text-[10px] font-bold leading-4 text-white">{n > 99 ? "99+" : n}</span> : null}
                </Link>
              </li>
            );
          })}
